...
public class EpisodesToPatternTransformer {
	...
	private Set<APIUsagePattern> makeArbitraryOrderExplicit(APIUsagePattern pattern) {
        Set<Set<Node>> components = getConnectedComponents(pattern);
        Set<APIUsagePattern> patterns = new HashSet<>();

        for (List<Set<Node>> permutation : Collections2.orderedPermutations(components, Comparator.comparingInt(Set::hashCode))) {
            APIUsagePattern patternPermutation = (APIUsagePattern) pattern.clone();
            for (int i = 0; i < permutation.size() - 1; i++) {
                for (Node source : permutation.get(i)) {
                    for (int j = i + 1; j < permutation.size(); j++) {
                        for (Node target : permutation.get(j)) {
                            patternPermutation.addEdge(source, target, new OrderEdge(source, target));
                        }
                    }
                }
            }
	...
	private void createComponent(Node node, APIUsagePattern pattern, Set<Node> component) {
		component.add(node);
